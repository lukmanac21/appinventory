...
class Pemasukan extends MY_Controller {
    ...
    var $meta_title = "INVENTORY | Manage Pemasukan";
    var $meta_desc = "INVENTORY";
    var $main_title = "Data Pemasukan";
    var $base_url = "";
    ...
    public function __construct() {
        parent::__construct();
        $this->base_url = $this->base_url_site . "pemasukan/";
    ...
    public function index() {
        ...
        $dt = array(
            ...
            "custom_js" => array(
                ...
                ASSETS_JS_URL . "form/pemasukan.js",
...
//            $no++;
            $row = array();
             $kain = $this->pemasukan_model->getDataKain(0, 'all', $dt->id);
//             echoPre($this->db->last_query());exit;
                $strkain = "";
                foreach ($kain as $value) {
                     $hasil_rupiah = "Rp " . number_format($value->harga,2,',','.');
                    $strkain .='<ul style="list-style: none;padding: 0px;"><li>Kain  <b>'.$value->nama. '</b> Jumlah Pemasukan <b>' .$value->jumlah.'</b> Harga Per QTY <b>'.$hasil_rupiah.'</b></li>';
                }  
            $row[] =$dt->nama;
//            $row[] = $dt->kain;
...
//            "draw" => $_POST['draw'],
            ...
            "recordsFiltered" => $this->pemasukan_model->count_filtered($nama),
            "data" => $data,
        );
    ...
    private function _homeEdit($id="") {
        ...
        $data = $this->pemasukan_model->getDetail($id);
...
//        echoPre($dt['kain']);exit;
        ...
        if(is_numeric($id)){
            $data = $this->pemasukan_model->getDetail($id);
//             echoPre($this->db->last_query());exit;
            $dataBarang = $this->pemasukan_model->getBarangDetail($id);     
            $dt['count'] = $this->pemasukan_model->getBarangCount($id);
            $resData = $data[0];
           ...
           ;
            $ret = $this->load->view("pemasukan/form_edit", $dt, true);
            
        }else {        
            $ret = $this->load->view("pemasukan/form", $dt, true);
        }
        return $ret;
    }
...
//        if (!empty($code)) {
            if (!empty($id)) {
                ...
                $res = $this->pemasukan_model->updateDetail($edit, $id);
                 $del_author = $this->pemasukan_model->deleteDetail($id);
                 $del_author['status'];
...
//                                "tanggal" => $tanggal,
                            ...
                            $resDet = $this->pemasukan_model->saveUpdateDataDetail($inDetail);
                        }
                    }
                } 
            ...
            // insert category
                ...
                $res = $this->pemasukan_model->saveData($insert);
                if ($res['status'] == true) {
                     // save detail barang
                    $index_row = isset($_POST["index_row"]) ? trim($_POST["index_row"]) : '';
                    for($i=1; $i <= $index_row;$i++){
...
//                                "tanggal" => $tanggal,
                            ...
                            $resDet = $this->pemasukan_model->saveDataDetail($inDetail);
                        }
                    }
...
//        }
 ...
 public function delete($id) {
        $del_author = $this->pemasukan_model->delete($id);
                      $this->pemasukan_model->deleteDetail($id);
        $del_author['status'];
   ...
   public function konfirmasi($id) {
      ...
      if($cek['status']==0) {
        ...
        $res = $this->pemasukan_model->updated($up, $id);
        if ($res['status'] == true) {
            $alert = array(
                "status" => "success",
