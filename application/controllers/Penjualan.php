...
class Penjualan extends MY_Controller {
    ...
    var $meta_title = "INVENTORY | Manage Penjualan";
    var $meta_desc = "INVENTORY";
    var $main_title = "Data Penjualan";
    var $base_url = "";
...
//        $this->base_url_redirect = $this->base_url_site . "Manage/Pemesanan/";
    ...
    private function _home() {
        ...
        $dt['warna'] = $this->penjualan_model->getWarna();  
        $dt["data"] = $data;
    ...
    public function ajax_list() {
        ...
        $list = $this->penjualan_model->get_datatables($nama);
...
//            $no++;
            $row = array();
             $kain = $this->penjualan_model->getDataKain(0, 'all', $dt->id);
            // echoPre($this->db->last_query());exit;
                $strkain = "";
                $warna = "";
                $jumlah = "";
                foreach ($kain as $value) {
                    ...
                    $strkain .='<ul style="list-style: none;padding: 0px;"><li><b>'.$value->produk. '</b></li>';
                    $warna .='<ul style="list-style: none;padding: 0px;"><li><b>'.$value->warna. '</b></li>';
                    $jumlah .='<ul style="list-style: none;padding: 0px;"><li><b>'.$value->jumlah. '</b></li>';
                }  
            $row[] =$dt->nama;
        ...
        $output = array(
            ...
            "recordsFiltered" => $this->penjualan_model->count_filtered($nama),
            "data" => $data,
        );
    ...
    private function _homeEdit($id="") {
        ...
        $dt["breadcrumbs"] = $this->setBreadcrumbs($arrBreadcrumbs);
        $dt["title"] = $this->main_title;        
        $dt['customer'] = $this->customer_model->check();
        $dt['produk'] = $this->penjualan_model->getProduk();
        $dt['warna'] = $this->penjualan_model->getWarna();  
...
//            
            ...
            $dataAgen = $this->customer_model->getDetail($resData['id_customer']);
            $dt["customers"] = $dataAgen;
...
//                    echoPre($_POST);exit;
                   for($i=1; $i <= $index_row; $i++){
                        ...
                        $ukuran = isset($_POST["ukuran_".$i]) ? trim($_POST["ukuran_".$i]) : '';
                        $motif = isset($_POST["motif_".$i]) ? trim($_POST["motif_".$i]) : '';
                        ...
                        if($jumlah != 0){
                            $inDetail = array(
                                ...
                                "ukuran" => $ukuran,
                                "motif" => $motif,
                                "total" => filterHarga($harga_barang),
                                "subtotal" => filterHarga($total_harga),
                            );
                            $resDet = $this->penjualan_model->saveUpdateDataDetail($inDetail);
                        }
                    }
                } 
