...
function loadDataAgen(id) {
    $.ajax({
        url: base_url+'getAgen/'+id
    })
    .done(function( msg ) {
        $("#alamat").val(msg.alamat);
        $("#no_telpon").val(msg.telp);
        $("#nama_customer").val( msg.nama);
    });
    return false;
}
function addBarang(){
   ...
   if(lastRow != idRow){
    ...
    htmlRow = htmlRow.split("ukuran_0").join("ukuran_"+idRow);
    htmlRow = htmlRow.split("motif_0").join("motif_"+idRow);
        ...
        setTimeout(function (){
            ...
            .done(function( msg ) {
                if(msg != null){
                    ...
                    $("#ukuran_"+indexRow).val(msg.ukuran);
                }
                else{
