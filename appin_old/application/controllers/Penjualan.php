...
//             echoPre($this->db->last_query());exit;
    ...
    private function _homeEdit($id="") {
        ...
        if(is_numeric($id)){
            ...
            $dataAgen = $this->customer_model->getDetail($resData['id_supplier']);
...
//                    echoPre($_POST);exit;
                   for($i=1; $i <= $index_row; $i++){
                        ...
                        $ukuran = isset($_POST["ukuran_".$i]) ? trim($_POST["ukuran_".$i]) : '0';
                        $motif = isset($_POST["motif_".$i]) ? trim($_POST["motif_".$i]) : '0';
