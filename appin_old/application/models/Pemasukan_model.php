...
class Pemasukan_model extends CI_Model {
    ...
    var $table = "tr_pemasukan";
    var $primary_key = "id";
    var $column_order = array('id', 'id', null);
    var $jointTable = "tr_pemasukan_detail";
//	var $column_search = array('txtProvinsi','txtKabupaten'); 
    ...
    public function getDataIndex($offset = 0, $limit = 10, $search = "") {
        ...
        $this->db->select('tr_pemasukan.id,tr_pemasukan.status,tr_pemasukan.tanggal,mst_supplier.nama,'
                . 'tr_pemasukan_detail.jumlah,tr_pemasukan_detail.total');
        $this->db->from($this->table);
        $this->db->join('mst_supplier','tr_pemasukan.supplier_id = mst_supplier.id','left');
        $this->db->join('tr_pemasukan_detail','tr_pemasukan.id = tr_pemasukan_detail.id_pemasukan','left');
        if ($limit != "all") {
            $this->db->limit($limit);
        }
    ...
    public function getDetail($id) {
        $this->db->select('tr_pemasukan.id,tr_pemasukan.supplier_id,tr_pemasukan.subtotal,tr_pemasukan.status,tr_pemasukan.tanggal,mst_supplier.nama,'
                . 'mst_supplier.alamat,mst_supplier.telp');
        $this->db->where('tr_pemasukan.id', $id);
        $this->db->join('mst_supplier','mst_supplier.id = tr_pemasukan.supplier_id','left');
//        $this->db->join('tr_pemasukan_detail','tr_pemasukan.id = tr_pemasukan_detail.id_pemasukan','left');
     ...
     public function getBarangDetail($id) {
        ...
        $this->db->select('*');
        $this->db->from('tr_pemasukan_detail');
          $this->db->join('mst_kain','mst_kain.id = tr_pemasukan_detail.id_kain','left');
...
//        $this->db->join('master_barang', 'master_barang.id_barang = '.$this->jointTable.'.id_barang');
        if(is_array($id)){
            $this->db->where_in("tr_pemasukan_detail.id_pemasukan", $id);
            $this->db->order_by("tr_pemasukan_detail.id_pemasukan", "asc");
        }
        else{
            $this->db->where("tr_pemasukan_detail.id_pemasukan", $id);
        }
    ...
    public function getDataKain($offset = 0, $limit = 10, $in = array()) {
        ...
        if (!empty($in)) {
            $this->db->where_in('id_pemasukan', $in);
        }
        $this->db->select('*');
        $this->db->from('tr_pemasukan_detail');
        $this->db->join('mst_kain', 'tr_pemasukan_detail.id_kain = mst_kain.id');
        $this->db->join('mst_jenis', 'mst_kain.kain_id = mst_jenis.id');
        if($limit != "all"){
            $this->db->limit($limit);
        }  
...
//        $this->db->where('tanggal', $tanggal);
        $q = $this->db->delete('tr_pemasukan_detail');
        if (!$q) {
    ...
    public function saveUpdateDataDetail($arrData = array(), $debug = false) {
        ...
        if ($debug) {
            ...
            $res = $this->db->insert('tr_pemasukan_detail');
            if (!$res) {
                $retVal['error_stat'] = "Failed To Insert";
                $retVal['status'] = false;
            } else {
    ...
    private function _get_datatables_query() {
         $this->db->select('tr_pemasukan.id,tr_pemasukan.status,tr_pemasukan.tanggal,tr_pemasukan.subtotal,mst_supplier.nama');
        $this->db->from($this->table);
        $this->db->join('mst_supplier','mst_supplier.id = tr_pemasukan.supplier_id','left');
        $i = 0;
