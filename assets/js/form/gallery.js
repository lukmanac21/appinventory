...
$('#modal-media-edit').on('show.bs.modal', function (event) {
  ...
  var id = button.data('id')
  $.ajax({
       url : base_url_trending+"detailGallery/",
       type : "POST",
       data : "id="+id,
       type : "post",
       dataType : "html",
       success: function msg(res){
            var data = jQuery.parseJSON(res);
            var modal = $(this);
            $('#form_id_image').val(data['ID']);
            $('#form_url_edit').val(data['link_image']);
            $('#form_title_edit').val(data['gallery_title']);
            $('#form_desc_edit').val(data['gallery_desc']);
            $('#gallery-img-edit').html(data['html_image']);
       }
  });
});
...
function updateToGallery(mode){
    $.ajax({
                     url : base_url_trending+"updateGallery/",
                     type : "POST",
                     data : $('#detail-media-edit').serialize(),
                     dataType : "html",
                     success: function msg(res){
                          var data = jQuery.parseJSON(res);
                          var status = data['status'];
